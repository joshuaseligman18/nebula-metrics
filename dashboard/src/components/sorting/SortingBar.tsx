...
const SortingBar: React.FC<SortingBarProps> = ({ cpuMinuteValues, onMinuteRangeChange }) => {
  ...
  return (
    <div className="bg-gray-200 p-4 h-100">
      <div className="mb-4">
        <label className="block text-gray-700 text-sm font-bold mb-2">Select Start Minute</label>
        <select
          className="border border-gray-300 rounded-md shadow-sm p-2"
          onChange={handleStartMinuteChange}
        >
          {cpuMinuteValues.map((value, index) => (
            <option key={index} value={value} selected={startMinute === parseInt(value)}>
              {`${value}`}
            </option>
          ))}
        </select>
      ...
      <div>
        <label className="block text-gray-700 text-sm font-bold mb-2">Select End Minute</label>
        <select
          className="border border-gray-300 rounded-md shadow-sm p-2"
        ...
        >
          {cpuMinuteValues
            ...
            .map((value, index) => (
              <option key={index} value={value} selected={endMinute === parseInt(value)}>
                {`${value}`}
              </option>
            ))}
        </select>
