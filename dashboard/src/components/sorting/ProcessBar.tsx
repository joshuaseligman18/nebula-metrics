...
const ProcessBar: React.FC<ProcessBarProps> = ({ pids, onSelectPid, allProcessesData }) => {
  const [selectedProcess, setSelectedProcess] = useState<any | null>(null);

  useEffect(() => {
    // Auto-select process 1 when the component mounts
    if (!selectedProcess && allProcessesData && allProcessesData.length > 0) {
      const process1 = allProcessesData.find((process: any) => process.pid === 1);
  ...
  const handlePidChange = (pid: number | null) => {
    onSelectPid(pid);
    if (pid !== null) {
      const process = allProcessesData.find((process: any) => process.pid === pid);
  ...
  const formatValue = (key: string, value: any) => {
    if (typeof value === 'number') {
      ...
      } else if (key === 'virtual_memory' || key === 'resident_memory' || key === 'shared_memory') {
  ...
  return (
    <div className="bg-gray-200 p-4 h-100">
      <div>
        ...
        >
          <option value="">Select a PID</option> {/* Handle null value explicitly */}
          {pids.map((pid) => (
